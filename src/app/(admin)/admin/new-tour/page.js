      ...
      groupPrices: [
        {
          ...
          price: 0,
          perPerson: true, // false means total price for group
        },
        {
          groupSize: 2,
        ...
        },
        {
          ...
          price: 0,
          perPerson: true, // true means price per person
        },
        {
          groupSize: "8-12",
    ...
    try {
      ...
      const tourDataForFirestore = {
        ...
        pricing: {
          ...
          groupPrices: tourData.pricing.groupPrices.map((group) => ({
            groupSize: group.groupSize,
            label: group.label,
            price: Number(group.price),
            perPerson: Boolean(group.perPerson),
          })),
          discount: {
...
function TourPreview({ tourData }) {
  const hasItinerary =
    tourData.itinerary && tourData.itinerary.length > 0 && tourData.itinerary[0].title !== "";

  // Add this inside the TourForm function component
  const getStartingPrice = () => {
    // Add safety checks
    if (
      !tourData.pricing?.groupPrices ||
      !Array.isArray(tourData.pricing.groupPrices) ||
      tourData.pricing.groupPrices.length === 0
    ) {
      return { price: 0, perPerson: false };
    }

    // Filter out groups with price > 0 to find actual minimum
    const validGroups = tourData.pricing.groupPrices.filter(
    ...
    <div className="bg-stone-900 rounded-xl overflow-hidden border border-stone-700 shadow-2xl hover:shadow-amber-500/10 transition-all duration-300">
        ...
        <div className="absolute bottom-0 left-0 w-full p-6">
          <div className="flex justify-between items-end">
            <div className="max-w-[70%]">
              ...
              <p className="text-amber-300 drop-shadow-md text-sm">
                {tourData.basicInfo.shortDescription}
              </p>
            </div>
            ...
            <div className="text-right">
              ...
              {tourData.pricing.discount.amount > 0 && (
                <div className="text-sm text-white/70 line-through mt-1">
                  ${(startingPrice.price + tourData.pricing.discount.amount).toFixed(2)}
                </div>
              )}
            </div>
          </div>
      ...
      <div className="p-6 space-y-8">
          ...
          <div className="grid md:grid-cols-1 lg:grid-cols-1  gap-4 mb-6">
            ...
            <div className="flex items-center space-x-3 p-3 bg-stone-700/20 rounded-lg">
              ...
              <div>
                ...
                <p className="text-white font-medium">
                  {tourData.basicInfo.duration} {tourData.basicInfo.durationType}
                </p>
            ...
            <div className="flex items-center space-x-3 p-3 bg-stone-700/20 rounded-lg">
              ...
              <div>
                ...
                {tourData.basicInfo.liveTourGuideLanguages &&
                  tourData.basicInfo.liveTourGuideLanguages.length > 0 && (
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div
            className={`bg-stone-800/50 rounded-xl p-6 border border-stone-700 ${
              tourData.basicInfo.destinations.length > 1 ? "" : "md:col-span-2 lg:col-span-2"
            }`}
          >
            <div className="flex items-center mb-4">
        ...
        <button className="w-full bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 text-white font-bold py-4 px-6 rounded-xl transition-all duration-300 transform hover:scale-[1.02] shadow-lg hover:shadow-amber-500/30 text-lg">
          Book Now - From ${startingPrice.price}
        </button>
      </div>
